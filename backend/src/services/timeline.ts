...
const VALID_STAGES: TimelineStageType[] = ['early', 'turning-point', 'stable', 'crisis', 'rebuild', 'peak'];
...
const TIMELINE_SYSTEM_PROMPT = `你是一位专业的时间线档案员。你的任务是将人物传记解析为3-6个关键人生阶段节点。

每个节点必须包含以下字段：
- timeLabel: 时间标签（如"青年时期"、"1976年"）
- ageLabel: 年龄标签（可选，如"20岁前后"）
- stageLabel: 阶段名称（如"反叛探索期"）
- stageType: 阶段类型，必须是以下之一：early, turning-point, stable, crisis, rebuild, peak
- keyEvent: 核心事件（一句话概括）
- summary: 阶段摘要（2-3句话）
- traits: 人格特征数组（1-5个关键词，如["叛逆","敏锐"]）
- values: 价值观数组（1-5个关键词，如["完美主义"]）
- tensions: 内在冲突数组（1-4个关键词，如["理想与现实的冲突"]）
...
export async function parseTimeline(
  ...
  try {
    const userPrompt = `请解析以下人物的传记，生成时间线节点：

人物姓名: ${input.displayName}
传记内容: ${input.biography.substring(0, 3000)}
...
请生成3-6个关键阶段节点。`;
    ...
    if (!result.ok) {
      throw new Error(result.error || 'DeepSeek 请求失败');
    }
    ...
    const nodes: TimelineNode[] = rawNodes.map((node: Record<string, unknown>, index: number) => ({
      nodeId: `${profileId}-${index + 1}`,
      timeLabel: String(node.timeLabel || `阶段 ${index + 1}`),
      ageLabel: node.ageLabel ? String(node.ageLabel) : undefined,
      stageLabel: String(node.stageLabel || '未命名阶段'),
      stageType: VALID_STAGES.includes(node.stageType as TimelineStageType)
        ? (node.stageType as TimelineStageType)
        : 'stable',
      keyEvent: String(node.keyEvent || ''),
      summary: String(node.summary || ''),
      traits: Array.isArray(node.traits) ? node.traits.map(String) : [],
      values: Array.isArray(node.values) ? node.values.map(String) : [],
      tensions: Array.isArray(node.tensions) ? node.tensions.map(String) : [],
      sourceEvidence: [{ quote: String(node.summary || '').slice(0, 80), sourceLabel: 'AI生成' }],
    }));
    ...
    draft = {
      ...
      subtitle: '从原始叙述中提炼的人生节点',
      category: 'self',
      biography: input.biography.slice(0, 260),
      highlights: nodes.map((node) => node.keyEvent).slice(0, 4),
      suggestedTopics: [
        `如果你是 ${input.displayName}，当前最关键的选择是什么？`,
        `${input.displayName} 该如何理解自己最在意的代价？`,
      ],
      nodes,
    };
  } catch (error) {
    console.warn('timeline generation fallback (DeepSeek):', error);
