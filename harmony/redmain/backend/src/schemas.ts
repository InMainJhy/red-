...
const debateJudgeScorecardSchema = z.object({
  agentId: z.string().min(1),
  displayName: z.string().min(1),
...
const debateVerdictSchema = z.object({
  ...
  scorecards: z.array(debateJudgeScorecardSchema).min(1),
});
...
export const createSummaryRequestSchema = z.object({
  arenaRunId: z.string().min(1),
  profileId: z.string().min(1).optional(),
  topic: z.string().min(1),
  summary: arenaSummarySchema,
  participants: z.array(personaSpecSchema).min(1),
  messageIds: z.array(z.string().min(1)),
  duration: z.number().int().min(0),
});

export const updateSummaryRequestSchema = z
  .object({
